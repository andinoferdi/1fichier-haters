...
def download(worker, payload={'dl_no_ssl': 'on', 'dlinline': 'on'}, downloaded_size=0):
    ...
    while downloading:
        ...
        if not html.xpath('/html/body/div[4]/div[2]/a'):
            ...
            try:
                parsed_url = urlparse(urlx) if isinstance(urlx, str) else None
                valid_url = bool(parsed_url and parsed_url.scheme in ('http', 'https') and parsed_url.hostname)
                if valid_url:
                    _ = parsed_url.port
            except ValueError:
                valid_url = False
            if not valid_url:
                logging.debug('Invalid direct link. Changing proxy.')
                i += 1
                continue
            ...
            try:
                ...
                logging.debug('Download request failed.\n'+f'{e}')
                i += 1
                continue
