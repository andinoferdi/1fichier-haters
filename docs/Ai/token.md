...
- Jika RTK tersedia, awali shell command dengan `rtk` (contoh: `rtk git status`, `rtk python -m unittest discover -s tests`). Untuk cmdlet/built-in yang tidak bisa dijalankan RTK langsung, gunakan `rtk proxy powershell -Command "..."`.
